    ...
    if not _RESOURCE_FORMAT_SCORES:
        ...
        with open(json_filepath) as format_file:
            try:
                ...
                raise ValueError('Invalid JSON syntax in %s: %s' % (json_filepath, e))
            ...
            for format_line in file_resource_formats:
                ...
                if not isinstance(score, int):
                    raise ValueError('Score must be integer in %s: %s: %r' % json_filepath, format_, score)
                if format_ in _RESOURCE_FORMAT_SCORES:
                    raise ValueError('Duplicate resource format identifier in %s: %s' % (json_filepath, format_))
